...
public sealed class LiveMonitor : IDisposable
{
    ...
    public const string PresentMonMissingNote =
        "No FPS data: PresentMon could not start. Check the Activity tab for the reason.";

    public bool IsRunning => _presentMon is { HasExited: false };

    public static string FindPresentMon(string configured) => PresentMonTool.EnsureAvailable(configured);
